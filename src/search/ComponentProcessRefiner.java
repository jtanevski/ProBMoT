...
public class ComponentProcessRefiner {
	 ...
	 */
	private ListMap<String, IPRecursiveRefiner> refiners = new ListMap<String, IPRecursiveRefiner>();
	...
	ComponentProcessRefiner(ExtendedModel extendedModel) {
...
*/
	 ...
	 * @param onlyTopLevel
	 */
	IPRecursiveRefiner(ExtendedModel extendedModel, IP ip, boolean onlyTopLevel) {
...
//			ExhaustiveSearch.logger.warn("Creating AE - check this behavior");
	...
	IP createIP(IAP ap) {												// creating IPs is the same regardless of whether it is iterated
		IP newIP = new IP();
		newIP.id = StringUtils.uncapitalize(ap.argTemp.id) + (int)(Math.random()*1000);
